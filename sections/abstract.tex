\Abstract
\textit{Oblivious Random Access Memory (ORAM)} provides secure access to storage, primarily in the context of databases, by obfuscating access patterns.
An attacker cannot infer any information from these access patterns.
\textit{Distributed ORAM (DORAM)} extends \textit{ORAM} with the partitioning of storage among multiple parties such that no single party can reconstruct the data by itself.
A fixed number of cooperating parties is required to reconstruct the data.
This research focuses on \textit{DUORAM}, a \textit{DORAM} implementation by Vadapalli et al. \cite{vadapalli2023duoram}.
...
Allowing the user to store and retrieve elements of arbitrary size enables the usage of \textit{DUORAM} in the context of databases, where elements of varying size are handled.
Various scaling approaches are discussed, along with all necessary technical foundations.
The proposed approaches are implemented and benchmarked.
...
\textit{Distributed ORAM (DORAM)} erweitert \textit{ORAM} um die Aufteilung eines Speichers zwischen mehreren Parteien, sodass keine Partei alleine die Daten rekonstruieren kann.
Es benötigt eine feste Anzahl kooperierender Parteien um das Geheimnis, beziehungsweiße die Daten, wiederherzustellen.
Diese Research betrachtet \textit{DUORAM}, eine \textit{DORAM} Implementierung von Vadapalli et al. \cite{vadapalli2023duoram}.
...
Durch die Skalierung kann \textit{DUORAM} als Datenbank verwendet werden, da in jenem Kontext häufig Elemente unterschiedlicher Größe verarbeitet werden müssen.
Die Ansätze werden implementiert und gebenchmarked.
