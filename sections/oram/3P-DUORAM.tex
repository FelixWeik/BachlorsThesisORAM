...
This research puts its focus solely on the 3-party variant.
\textit{DUORAM} uses \textit{(2,2)-DPFs} (\ref{sub:DistributedPointFunctions}) to represent \textit{Private Information Retrieval (PIR)} and \textit{PIR-Writing} queries (\ref{sub:PrivateInformationRetreival}).
...
Each party holds three \textit{RDPFs} in form of \textit{RDPF-Triples}.
...
3-Party \textit{DUORAM} uses (2+1)-MPC protocols to replace the trusted source \cite{vadapalli2023duoram}.
...
If not specified differently, refreshing the blinds will just be adapted to the given data size.
...
Depending on the parameters given, one can also generate \textit{DCFs} (\ref{sub:DistributedComparisonFunctions}), \textit{Multiplication Triples}, and \textit{Addition Triples} (\ref{sub:BeaverTriples}).
Each RDPF-Triple is generated on the same share, thus all three \textit{DPFs} by $P_0$ have the same index, and all \textit{DPFs} held by $P_1$ share the same index too.
Of course, these do not have to be equal.

\textit{DUORAM} uses a (2+1)-Party MPC protocol for generating their \textit{DPFs}, which allows to replace the trusted source by a stateless third party $P_2$.
Both parties $P_0,P_1$ choose a random index $\widetilde{ri}_0,\widetilde{ri}_1$ with $\widetilde{ri}_0\oplus\widetilde{ri}_1=ri$.
The \textit{DPFs} will be generated on these target indices using the BGI-construction (\ref{sub:BGIDPFConstruction}).
...
$P_2$ holds the \textit{DPFs} $(\overline{k}_0^2, \overline{k}_1^3)$.
...
Therefore, one must set the \textit{Final Correction Word} $F$ to change the random result into the message $M$.

$P_0,P_1$ exchange $M_b+F_b$ (with $F$ being the \textit{Final Correction Word}).
This allows both parties to reconstruct $M-(v_0[i^*]+v_1[i^*])$ which will serve as the new \textit{Final Correction Word} $F'$ to write $M$ in the desired location.
...
Recall $t$ being the bit-vector holding the flagbit of leaf $i$ at position $i$, $v$ being the vector holding all leaf labels and $F$ being the final-correction-word (\ref{sub:BGIDPFConstruction}).
$t_b^k$ has the property of $e_{i^*}=t_0^1+t_1^1=t_0^2+t_1^2=t_0^3+t_1^3$ with $e_{i^*}$ being the shifting vector. 
...
Figure 5 in the research of Vadapalli et al. \cite{vadapalli2023duoram} provides a nice overview over the reading online protocol.
...
Advanced preprocessing, again, does not extend the already defined preprocessing from \ref{subsub:DuoramPreprocessing}.
This means $P_0,P_1$ generate the same RDPF-Triples on random indices and distribute them as above. 
However, more parameters are extracted compared to the reading operation (\ref{par:advancedReadingPreproc}).

During RDPF generation $P_0,P_1$ learn XOR-shares of the random \textit{Final Correction Words} (\ref{sub:BGIDPFConstruction}) $F_0^k,F_1^k\text{ with }k\in\{1,2,3\}$.
...
Note that despite the \textit{Final Correction Words} being XOR-shared they are not be converted to additive shares as the RDPF itself stays XOR-shared.
...
These values allow the peers to reconstruct three \textit{Final Correction Words} $F^k\leftarrow(M_0+F_0^k+M_1+F_1^k)$ and the server and the peers to reconstruct the offset $S\leftarrow(i_0^*+ri_0+i_1^*+ri_1)$.
...
$P_2$ is not directly involved in the updating process. 
