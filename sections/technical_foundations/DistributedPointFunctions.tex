...
The vanilla implementation of Vadapalli et al. \cite{vadapalli2023duoram} creates DPFs in a way which will keep this tree-invariant intact by design.
...
This can be easily implemented by keeping the flag bits on node pairs on the path unequal while having them equal in node pairs not on the path.
...
XORing $v_0\oplus v_1 = e_\alpha\cdot\beta$ with $\beta$ results in the target value. 
...
DUORAM's \textit{Online Phase} (\ref{subsub:DuoramOnlinePhase}) uses such protocol to either postpone the decision of where to read/write or postpone the decision of what to write.
