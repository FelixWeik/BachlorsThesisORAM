...
The \textit{FSS} (\ref{ch_FunctionSecretSharing}) for secretly sharing Comparison Functions is called \textit{Distributed Comparison Function (DCF)}.
...
Using Point Functions (\ref{sub:PointFunctions}), one is interested in the flag bits, correction words (including the final correction word), and the label of the leaf.
...
Multiple DCF constructions coexist.
...
For this purpose one uses a pair of the already mentioned DPFs (\ref{sub:DistributedPointFunctions}), and expands them to meet the functional requirements of DCFs.
...
If one now evaluates the DPF on $target$, which is equal of calculating $V[target]$, one receives a value of either $0$ or $1$.
Assume $V[target]=1$.
