...
Yet, since trips for a customer are always inserted at the same location, by observing the access patterns and using additional background information a transportation provider is still able to infer the user that created the trip and potentially also the trip information.
...
To prevent such conclusions, one must not only hide the data itself but also conceal access patterns to the database.
...
Chapter \ref{ch:TechnicalFoundations} lays the mathematical and conceptual fundamentals for understanding \textit{ORAM} and \textit{DUORAM} introduced in chapter \ref{ch:ORAM}.
...
The results are then used in chapter \ref{ch:Conclusion} to answer the research questions and to highlight unanswered questions as well as potential approaches for future research.
...
Vadapalli et al. \cite{vadapalli2023duoram} chose to reduce the communication complexity by generating DPFs locally.
