...
These improvements come in form of the \code{wmmintrin.h} header file implementing AES-ECB (Advanced Encryption Standard in Electronic Code Book Mode) core functions based on the \textit{AES-NI} instruction set.
...
Therefore, datatypes used in any AES-context are stored in this format.

The careful reader notices \code{\_\_m128i} vectors being twice as large as the \code{uint64\_t} values the client wants to store and retreive in the vanilla implementation.
This is due to AES-ECB being used as a pseudo random generator during creation and traversion of DPF-trees.
The code uses its security properties of being random to calculate new labels given a seed and key.
...
Therefore, by using a \code{\_\_m128i} seed and labels encrypting each label using AES-ECB recursively, one simluates a length doubling pseudo random generator.
