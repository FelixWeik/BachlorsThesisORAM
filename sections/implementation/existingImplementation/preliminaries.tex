...
Thus, the most trivial representation of a single DPF is saving the seed of the root node and generating the whole tree every time the point function is used.
...
All three parties execute the same executable \code{prac}.
...
\begin{itemize}
    ...
    Note that this requires the preprocessing mode to be executed first since it requires RDPF-triples for reads and writes.
    ...
    This means no preprocessing takes place, all needed resources are computed on the fly.
