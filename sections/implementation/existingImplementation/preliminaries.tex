...
More information on \code{LeafInfo} and the concept of \code{DPFNode}s and \code{LeafLabel}s is given in \ref{subsub:DpfNodeAndLeafLabel}.
...
Therefore, by using a \code{\_\_m128i} seed and labels encrypting each label using AES-ECB recursively, one simulates a length doubling pseudo random generator \cite{vadapalli2023duoram}.
...
This structure is therefore stored in format of an \code{\_\_m128i[11]} array.
The seed at index $0$, the ten sub-keys on the following indices.
...
One practical drawback of using \code{\_\_m128i} vectors is its debugging incompatibility when using \textit{Valgrind}\footnote{More information can be found on their website and their documentations on https://valgrind.org/}.
\textit{Valgrind} is a debugging suite designed for spotting memory leakages in the code.
It is, however, not compatible for using SIMD datatypes which triggers an error message and ultimately stopping the program.
Thus, debugging becomes tedious and unnecessarily hard.

\subsubsection*{DPFNode and LeafLabel}\label{subsub:DpfNodeAndLeafLabel}
%//TODO hier gehts weiter lieber Felix, jetzt implementier mal ein wenig 
...
For example, when executing multiple database accesses multiple RDPF-Triples and thus separate contexts are needed.
Instead of assigning these contexts to the same database introducing a great overhead by updating the database, assigning new RDPF-Triples and \code{mpcio} objects, one simply updates the underlying shape introducing a fresh context.
The \code{context} operation updates the shape's context, generates a new shape of the same type with the same underlying database and returns the newly generated object.
Only the \code{Flat} shape is used in the implementation.
Therefore, no further descriptions on different shapes and shape hierarchies are provided.
