\subsection*{Representing DPFs} \label{subsub:implementationRepresentingDPFs}
Both, \textit{Distributed Point Functions} (\ref{sub:DistributedPointFunctions}) and \textit{Distributed Comparison Functions} (\ref{sub:DistributedComparisonFunctions}), inherit from the same parent class \code{dpf}.
Recall the representation of \textit{DPFs} and \textit{DCFs} as binary trees.
Thus, the most trivial representation of a single \textit{DPF} is saving the seed of the root node and generating the whole tree every time the point function is used.
Since this approach is too computational expensive for repetitive evaluations, \textit{DPFs} are stored in an already evaluated form with the possibility to recreate the tree when needed.
...
The code further defines special \textit{DPFs} namely \textit{Distributed Point Functions for Memory Access (RDPF)} and \textit{Distributed Comparison Functions (CDPF)}.

\paragraph{RDPFs}\label{par:preliminariesRDPFs}
RDPFs inherit from the general \code{dpf} implementation.
It extends its parent with further attributes and the \code{LeafInfo} struct.
...
RDPFs of $\code{WIDTH}=1$, which is the default case in this research, hold a single \code{LeafInfo} struct.
...
As the name suggests, it holds three RDPFs, each with the same target.
This target is stored in both forms, additively and XOR shared.
...
This means it holds 64-bit for writing, which adds up to $M_b$ at the target index, and 64-bit for reading, adding up to the shifting vector when combined with the other peer's shifting vector.
...
\subsection*{types.hpp} \label{subsub:implementationTypeshpp}
...
\begin{itemize}\label{itemize:importantAliases}
    \item \code{nbits\_t}: 
    Counts the number of bits in a value, which means \code{value\_t}.
    ...
    It is set to \code{uint32\_t} by default.
    \item \code{DPFnode}: 
...
\begin{itemize}\label{itemize:importantMacros}
    \item {VALUE\_BITS}: 
    Decides the size of \code{value\_t}. 
...
\subsection*{Intel AES-NI} \label{subsub:implementationAesni}
...
As this header file is included in the gcc compiler, no additional installations are necessary.
...
After inserting the seed which resembles the key of the AES operation, ten more sub-keys are generated starting from the seed.
...
A non-standard value of \code{LWIDTH}, which means any value $x\neq1$, requires multiple keys in the leaf layer as several leaf labels need to be instantiated (\ref{subsub:DpfNodeAndLeafLabel}).
As this matter is only present in the leaf layer, using separate keys during generation increase performance and readability of the code.
...
The author uses \textit{ASan}\footnote{More information can be found in the documentation by Microsoft Learn: https://learn.microsoft.com/de-de/cpp/sanitizers/asan?view=msvc-170} instead.

\subsection*{Duoram and Shapes} \label{subsub:implementationDuoram}
...
\begin{itemize}
    ...
    Its size is the same as the database.
\end{itemize}

Each \code{duoram} must be accessed via a hierarchy of \textit{Shapes}.
The implementation uses a hierarchy consisting of a single shape called \code{Flat}.
...
Using shapes provides the great advantage of updating independent accessing contexts while keeping the underlying database consistent.
...
This reduces the overhead to a minimum when executing independent reads and writes.

The base implementation uses the \code{Flat} shape only.
Extensions on different shapes are not planned or necessary. 
Thus, no further descriptions on different shapes and shape hierarchies are required and provided.

\subsection*{WIDTH and LWIDTH}\label{subsub:WIDTHandLWIDTH}
RDPFs can be reused for reading.
This, however, leaks the information that reads were executed on the same location multiple times.
The location itself stays secret \cite{vadapalli2023duoram}.
...
Vadapalli et al. template their \code{RDPF} struct with a property called \code{WIDTH} of type \code{nbits\_t} (\ref{itemize:importantAliases}).
\code{WIDTH} is set to $1$ by default.
This indicates one write access per RDPF, no reuse is possible.
Increasing this value to $n$ adjusts the type aliases \code{RegASW} and \code{RegXSW}, both containing $n$ instances of \code{RegAS} or \code{RegXS} respectively.
...
As it is of type \code{nbits\_t} (\ref{itemize:importantAliases}) setting \code{WIDTH} to $1$ sets \code{LWIDTH} to $1$ too.
...
Performing either a single read or write requires the array to contain only a single \code{\_\_m128i} value.
Increasing \code{WIDTH} to $n$ leads to $n$ 64-bit values being needed for $n$ writes.
Therefore, the array representing leaf nodes must be adjusted to hold $n+1$ 64-bit values.
...
\subsection*{DPFNode and LeafNode}\label{subsub:DpfNodeAndLeafLabel}
The concepts of DPF nodes and leafs, \code{WIDTH}, and \code{\_\_m128i} vectors are closely intertwined.
...
This is done by using AES-ECB on vectors twice as large as the actual label, which means 128-bits.
...
In short, each leaf is divided into $2n*$\code{LWDITH} blocks of size 64, which makes it reusable for \code{WIDTH} many updates.
This means, to implement a DPF tree with a certain value of \code{WIDTH}, one needs a leaf label of size $\code{LWIDTH}*128$.
Thus, \code{LeafNode} is the type alias for a vector holding \code{LWIDTH} many \code{\_\_m128i} values or \code{DPFnode}s respectively.
This results in the type definition \code{using LeafNode = std::vector<DPFNode, LWIDTH>}. 

\subsection*{mpcio and Communication} \label{subsub:implementationMpcioAndCommunication}
All three parties run the same executable \code{prac}.
It is generated using a \code{MAKEFILE}.
...
It is one of the first structs to instantiate and is used to create most of the other objects.

\code{MPCPeerIO} is the child of the \code{mpcio} struct which holds general IO-attributes and provides functionality for timing the communication process and round-trip-times.
This struct also holds information about the chosen processing mode.
The program offers three modes: 
\begin{itemize}
    \item -p: 
    Defines the \code{PRECOMPUTATION\_MODE}, which executes the preprocessing phase as explained in section \ref{subsub:DuoramPreprocessing}.
    \item Leaving the specifier blank: 
    Executes the \code{ONLINE\_MODE}.
    Note that this requires the preprocessing mode to be executed first as it requires RDPF-Triples for reads and writes.
    The user can specify which tests to execute.
    The default test is called \code{duoram}.
    It executes reads and writes, each independent and interleaved.
    \item -o:
    Executes the \code{ONLINE\_ONLY} mode.
    This means no preprocessing takes place, resources are computed on the fly.
    Of course, this impacts performance but will come in handy for performing smaller tests.
    With this specifier, too, one can choose which tests to execute.
    These tests are be the same as in the online mode.
\end{itemize}

All three parties use a predefined message structure.
Generated preprocessing data (\ref{subsub:implementationPreprocessing}) implements this protocol.
It separates a message into byte-segments with each segment accommodating a special type of data:
\begin{itemize}\label{itemize:preprocessingStructure}
    ...
    \item 0x83: Select Triple. Since it is not used further the benevolent reader may read the comments in \code{types.hpp}.
