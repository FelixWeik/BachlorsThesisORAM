...
Having established a theoretical foundation of the DUORAM construction, this section further dives into the protocols involved in reading and writing.
After taking a closer look into the protocols, this section conducts a theoretical analysis of how to arbitrarily scale DUORAM to fulfil the objective of this research.
...
Fully evaluating both triples would each return three vectors $v_b^k,t_b^k,F_b^k \text{ with }b\in\{0,1\},k\in\{1,2,3\}$.
...
This provides consistency throughout the implementation as the database itself, too, is additively shared and therefore does not need to be converted to XOR-shares.
...
This means reading first executes cyclic shifting to decide where to read and then the basic reading protocol. 
...
In short, message $M$ will be written at position $i^*$ into the shared database $D$ resulting in the new database $D'$ which, again, is shared among the peers.
...
\begin{flalign*}
   D_0[i^*]+D_1[i^*]+(v_0^1[i^*]+t_0^1[i^*]\cdot F^1)+(v_1^1[i^*]+t_1^1[i^*]\cdot F^1) \\
    ...
    = D'[i^*] 
\end{flalign*}
and staying at the initial unchanged value of $D[i]$ at every other $i\neq i^*$.

This means $P_2$ is not directly involved during updating but only updates the peers' blinds for the next read operation.
